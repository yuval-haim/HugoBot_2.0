...
class TemporalAbstraction:
        ...
        if method_config is not None:
            ...
            if per_entity:
                if split_test:
                    ...
                    final_states = self._entity_state_mapping
            else:
                if split_test:
            ...
            if split_test:
                if method == "equal_width":
                    ...
                    ta_method = KnowledgeBased(**kwargs)
                else:
    ...
    def _composite_fit_transform_entity(self, train_data: pd.DataFrame, method_config: dict):
        ...
        for ent, ent_group in train_data.groupby(ENTITY_ID):
            # Lookup configuration for this entity; keys can be entity IDs or use "default"
        ...
        for tpid, subset in data.groupby(TEMPORAL_PROPERTY_ID):
            ...
            for cfg in cfgs:
                ...
                if method_name == "equal_width":
                    ...
                    from .methods.knowledge import KnowledgeBased
                    kb_inst = KnowledgeBased(**params, per_variable=True)
                    local_result = kb_inst.fit_transform(subset_method)
                    local_states = kb_inst.get_states()
                else:
    ...
    def _composite_transform_entity(self, test_data: pd.DataFrame, method_config: dict):
        """
        ...
        """
        entity_results = []
        for ent, group in test_data.groupby(ENTITY_ID):
            if ent in method_config:
                cfg = method_config[ent]
            else:
                cfg = method_config.get("default")
            if not isinstance(cfg, list):
                cfg = [cfg]
            ent_result = self._composite_transform_entity_single(group, cfg)
            entity_results.append(ent_result)
        final_test = pd.concat(entity_results, ignore_index=True)
        return final_test

    def _composite_transform_entity_single(self, data: pd.DataFrame, config):
        """
        ...
        for tpid, subset in data.groupby(TEMPORAL_PROPERTY_ID):
            ...
            for cfg in cfgs:
                ...
                if method_name == "equal_width":
                    from .methods.equal_width import equal_width
                    local_result, _ = equal_width(subset_method, **params, per_variable=True)
                elif method_name == "equal_frequency":
                    from .methods.equal_frequency import equal_frequency
                    local_result, _ = equal_frequency(subset_method, **params, per_variable=True)
                elif method_name == "sax":
                    from .methods.sax import sax
                    local_result, _ = sax(subset_method, **params, per_variable=True)
                elif method_name == "gradient":
                    from .methods.gradient import gradient
                    local_result, _ = gradient(subset_method, **params, per_variable=True)
                elif method_name == "td4c":
                    from .methods.td4c import TD4C
                    td4c_inst = TD4C(**params, per_variable=True)
                    local_result = td4c_inst.fit_transform(subset_method)
            ...
            composite_results.append(var_result)
        final_entity_result = pd.concat(composite_results, ignore_index=True)
    ...
    def _composite_fit_transform(self, train_data: pd.DataFrame, method_config: dict):
        ...
        for tpid in unique_vars:
            ...
            configs = method_config.get(tpid, None)
            if configs is None:
                if default_config is None:
                    raise ValueError(f"No configuration for variable {tpid} and no default available.")
                else:
                    configs = default_config if isinstance(default_config, list) else [default_config]
            local_global_states = {}
            for config in (configs if isinstance(configs, list) else [configs]):
                ...
                if method_name == "equal_width":
                    ...
                    from .methods.knowledge import KnowledgeBased
                    local_result, local_states = KnowledgeBased(states=params.get("states"), per_variable=True).fit_transform(subset), KnowledgeBased(states=params.get("states"), per_variable=True).get_states()
                else:
                    raise ValueError(f"Unsupported method: {method_name} for variable {tpid}")
                if isinstance(local_states, dict):
                    boundaries = local_states.get(tpid)
        ...
        for tpid in unique_vars:
            ...
            for config in (configs if isinstance(configs, list) else [configs]):
                ...
                if method_name == "equal_width":
                    ...
                    from .methods.knowledge import KnowledgeBased
                    local_result, _ = KnowledgeBased(states=params.get("states"), per_variable=True).fit_transform(subset), KnowledgeBased(states=params.get("states"), per_variable=True).get_states()
                else:
                    raise ValueError(f"Unsupported method: {method_name} for variable {tpid}")
                temp_mapping = {}
                temp_states_rows = []
    ...
    def _save_results(self, output_dir: str, symbolic_series: pd.DataFrame, states, max_gap: int):
        ...
        def sort_key(x):
            # Assume that numeric keys come first, then any others.
            if isinstance(x, int):
                return (0, x)
            else:
                return (1, str(x))
        ...
        if isinstance(states, list) and states and "MethodName" in states[0]:
            ...
            global_mapping = {}
            states_rows = []
            # Instead of sorted(states.keys()), we now sort with a custom key.
        ...
        # Additional KL Files by Entity Class.
