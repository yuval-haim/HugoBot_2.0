...
def paa_transform(data: pd.DataFrame, window_size: int = 3, agg_method: str = 'mean') -> pd.DataFrame:
    """
    ...
    into non-overlapping windows of size `window_size`, and computes aggregated values and timestamps.
    
    Parameters:
      data: pd.DataFrame
      window_size: int, window length.
      agg_method: str, one of 'mean', 'min', or 'max'.
    ...
    def paa_group(group):
        ...
        for start in range(0, n, window_size):
            ...
            aggregated_time = window[TIMESTAMP].mean()
            row = {
    ...
    return data.groupby([ENTITY_ID, TEMPORAL_PROPERTY_ID], group_keys=False).apply(paa_group)
...
def generate_KL_content(symbolic_series: pd.DataFrame, max_gap: int) -> str:
    ...
    for entity in entities:
        ...
        entity_line = f"{entity};" + ";".join(interval_strs) + ";"
